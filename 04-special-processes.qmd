...
**Work in progress.** This chapter collects processes with a richer input–output
structure than the power plants of chapters 1–3. The electrolyser and vehicle
examples now load **real technology specifications** from `tech_specs/`; the blast
furnace is still a structural sketch, and every spec should be run through
`tech_spec_issues()` before it is used in a solving model.
:::
...
coke. This one is still a **structural sketch** — adapt the coefficients from the
IDEEA iron-and-steel model (`WRI/IDEEA-Internal-Repo/IDEEA-IIS.qmd`, which defines
BF/BOF/EAF/DRI).
...
::: {.callout-note appearance="simple"}
Input **groups** (interchangeable inputs bounded by shares) are illustrated here
but are **not yet solvable** in the shipped model — keep the blast furnace as a
*structure* exercise (`draw()`, read the RES) rather than a scenario that must
solve, until the grouped input–output balance fix lands.
:::

```{r}
#| eval: false
# TODO: coefficients / costs from the IDEEA I&S model
...
- **Groups + shares** (blast-furnace fuels, vehicle highway/city split) are the
  structural step beyond chapters 1–3; note the solvability caveat above.
